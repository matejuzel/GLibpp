...
Mesh ObjLoader::LoadFromString(const std::string& text)
{
	...
	while (!rest.empty()) {
		...
		if (keyword == "v") {
			...
			msh.vertexBuffer.emplace_back(x, y, z, 1.0f);
		}
		else if (keyword == "f") {
			faceIndices.clear();
			for (std::string_view token = nextToken(line); !token.empty(); token = nextToken(line)) {
				faceIndices.push_back(parsePositionIndex(token, msh.vertexBuffer.size(), lineNo));
			}
			...
			for (size_t i = 1; i + 1 < faceIndices.size(); ++i) {
				msh.indexBuffer.push_back(faceIndices[0]);
				msh.indexBuffer.push_back(faceIndices[i]);
				msh.indexBuffer.push_back(faceIndices[i + 1]);
			}
		}
		else if (keyword == "mtllib" || keyword == "usemtl") {
			// materialy zatim nepodporujeme; az Mesh dostane materialy/sub-range,
			// tady se sesbira jmeno .mtl souboru a hranice usemtl skupin
		}
		// vt, vn, vp, s, o, g, l, p a nezname keywordy tise preskakujeme
	}

	if (msh.vertexBuffer.empty() || msh.indexBuffer.empty()) {
