...
public:
    ...
    // Vloûit nahoru pro _mm_pause()
#include <immintrin.h> 

    void waitUntilNextStep(double slack = 0.002) { // Zvednut˝ slack na 2 ms!
        ...
        // 1. F¡ZE: OS Sleep (uvolnÏnÌ CPU pro ostatnÌ procesy)
        // Pokud m·me k dobru vÌce neû 2 ms, m˘ûeme si dovolit riskantnÌ usnutÌ
        if (timeToWait > slack) {
            ...
            // Po probuzenÌ zjistÌme, kde p¯esnÏ jsme
            updateAccumulatorOnly();
        }

        // 2. F¡ZE: Spin-lock (AbsolutnÌ p¯esnost na mikrosekundy)
        // Tady uû nesmÌme usnout, zbytek Ëasu "protopÌme" aktivnÌm Ëek·nÌm
        while (m_accumulator < m_fixedDelta) {
            // _mm_pause() ¯ekne procesoru, ûe jde o spin-lock. 
            // Drasticky to sniûuje spot¯ebu a zah¯Ìv·nÌ CPU bÏhem pr·zdnÈ smyËky.
            _mm_pause();
