...
- **Render-to-texture works through the same texture residency.** A texture's residency target IS a render target: `device.textureTargetGet(TextureHandle)` resolves the handle to the backend `TargetHandle`, which a pass binds with `SetFramebuffer` (plus its own `SetDepthbuffer`/`SetViewport` — the rasterizer takes dimensions from the **viewport**, not the target) and draws into; the next pass samples it via plain `SetTexture` with zero copying (`framebuffer` points into `texelStorage`). The Renderer resolves the RT target lazily on the render thread (first frame with a valid handle — residency exists only after the upload walk) and owns the RT depth target. Demo: `buildDrawList` pass 0 renders the car into `rtTexture` (128×128), shown on `rtPanel`. Keep RT passes lean: the RT pass in the demo deliberately skips the heavy meshes (wave ~7k, icosphere ~5k, line.obj ~4.6k triangles — measured per-command: each alone tips Debug frames into 33 ms pacing) and the RT panel itself (feedback loop — it would sample the texture being written).
