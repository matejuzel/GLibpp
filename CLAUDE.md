...
- **Render-to-texture works through the same texture residency.** A texture's residency target IS a render target: `device.textureTargetGet(TextureHandle)` resolves the handle to the backend `TargetHandle`, which a pass binds with `SetFramebuffer` (plus its own `SetDepthbuffer`/`SetViewport` — the rasterizer takes dimensions from the **viewport**, not the target) and draws into; the next pass samples it via plain `SetTexture` with zero copying (`framebuffer` points into `texelStorage`). Any future RT pass must stay lean (measured per-command: re-drawing the wave ~7k, icosphere ~5k or line.obj ~4.6k triangles each alone tips Debug frames into 33 ms pacing) and must not draw the panel that samples its own texture (feedback loop).
- **The demo shows the cheaper sibling of RT: end-of-frame capture.** `Renderer::captureFrame` (after submit, before present) copies the finished framebuffer via `device.targetCopyColor` (memcpy) and converts the depth target to grayscale via `device.targetCopyDepthGray` (near = light, far/clear = black; gamma ¼ via double `sqrt` — NDC z is so nonlinear that a linear scale renders almost everything black) into two capture textures shown on `fbPanel`/`depthPanel` — **one frame of latency by design** (a live framebuffer cannot be sampled: the clear at frame start destroys the previous image). The float→ARGB conversion kernels `convertDepthToGrayScalar/SSE2/AVX2` are free functions in `DeviceTargetDIB.h` (unit-tested without a window; all variants truncate so outputs are bit-identical), dispatched by `cpuFeatures` like `clearFill`. Capture targets follow the window size via `targetResize` in `syncCaptureSize` (the canonical `TextureData` stays deliberately stale — content is residency-only). Panel UVs stay 0..1 (image stretched to the square panel; fit/letterbox via aspect-initialized UVs + a border-black sampling shader is a noted future step). Wireframe lines don't write depth, so the depth panel shows only filled geometry.
- **Texture filtering: bilinear + anisotropic via RIP-maps.** Three sampler shaders share one code path: `Textured` (nearest, exact — what the raster tests rely on), `TexturedBilinear` (4 taps on level 0), `TexturedAniso` (RIP level selection + bilinear inside it). A **RIP-map** (`RipMapDIB.h`) is a 2D table of levels `W/2^i × H/2^j` with **independent** halving per axis, packed into one array with an `[j*levelsU + i]` offset table (same offsets-in-a-big-array idiom as mesh residency); the `i == j` diagonal is the ordinary mip chain. Cost: 4·W·H memory vs 1.33 for mips, but **one lookup per pixel** instead of N taps along the major axis — the CPU trade is inverted vs a GPU (per-pixel work is scarce, memory is cheap). Limitation: only axis-aligned anisotropy; diagonal elongation picks the conservative level (EWA/N-tap is deliberately out of scope). **Derivatives need no 2×2 quads**: the rasterizer interpolates `u/w, v/w, 1/w` with plane equations, so `ShaderTexturedAniso::setup` computes the gradients analytically (`screenGradient`) and `shade` gets exact `du/dx = w·(A'x − u·C'x)` — hence no change to `TriangleInput`, `PixelInput`, or the shader contract, and other shaders pay nothing. LOD comes from the float exponent (`log2FloorClamped`), not `log2` (a CRT call per pixel in `/Od`). Levels are built at upload **only for static assets**: `Assets::TextureData::dynamic` marks capture/RT textures, whose levels would go stale after the first frame — those sample the target directly with `levelsU == levelsV == 1`, so the sampler needs no branch. **Measured**: filtering all three demo panels costs 60 → 31 FPS in Debug, so only the asset panel uses `TexturedAniso`; the capture panels stay nearest (pixel-exact is also the right thing for a diagnostic view).
