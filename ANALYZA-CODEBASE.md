...
- **Zbývá v rasterizéru: subpixel přesnost + exclusive fill rule.** Hloubka je teď vzorkovaná správně, ale *pokrytí* se pořád určuje ze zaokrouhlených vrcholů a spany jsou inclusive na obou koncích, takže pixel na sdílené hraně kreslí oba trojúhelníky (deterministicky vyhrává první, žádné švy — ale je to plýtvání a hrany se při pohybu kamery cukají). Standardní řešení je jedno: nechat vrcholy ve floatu, testovat pokrytí ve středu pixelu a přidat top-left fill rule, aby každý pixel patřil **přesně jednomu** trojúhelníku. Znamená to přepsat scanline chůzi na float hrany (nebo přejít na edge-function/barycentrickou rasterizaci) — tím zmizí i „vertex crawling" a `edgeInterp` s celočíselným dělením.
