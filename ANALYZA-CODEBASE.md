...
> **Aktualizace 2026-08-13:** **filtrování textur** — bilineární vzorkování (`lerpARGB` po dvou kanálech v 16bitových pruzích registru) a **anizotropní filtrace přes RIP-map** (`RipMapDIB.h`: 2D tabulka úrovní `W/2^i × H/2^j` s nezávislým půlením po osách, packed + offset tabulka, box filtr `avgARGB` přes SWAR identitu). Derivace UV se počítají **analyticky** z gradientů rovin v `setup()` — žádné 2×2 quady jako na GPU, takže `TriangleInput`, `PixelInput` ani kontrakt shaderů se neměnily a ostatní shadery nezdražily. Výběr úrovně jde z exponentu floatu (`log2FloorClamped`), ne přes `log2`. Úrovně se staví jen pro statické assety (`TextureData::dynamic` odlišuje capture/RT textury). Testy 95 → 123 (mísení ARGB, layout a obsah úrovní vč. NPOT, bilineární tap, a hlavně **nezávislý výběr úrovně po osách** proti syntetické tabulce s markerovými barvami). Změřeno: filtrace všech tří panelů stáhla Debug z 60 na 31 FPS → aniso nese jen assetový panel, capture panely zůstávají nearest (u diagnostiky je pixel-exaktní pohled správnější).
